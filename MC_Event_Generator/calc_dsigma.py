...
class Calc_Dsigma(object):
	...
	def dsigma(self, costh):
		...
		PREFAC = (2 * math.pi) * param_card.alpha**2 / (4 * run_card.hats) # 2 * pi comes from d-phi integral
		return  PREFAC * ( A0 * ( 1 + costh**2 ) + A1 * costh )

		# return (4*param_card.alpha**2*math.pi**2*(4*param_card.MZ**4*(-1 + param_card.sw2)**2*param_card.sw2**2 + 2*run_card.ECM**2*param_card.MZ**2*param_card.sw2*(-1 - 7*param_card.sw2 +
